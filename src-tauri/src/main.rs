...
#[derive(Default)]
struct SpotifyPollerContainer(Mutex<spotify::SpotifyPoller>);
...
async fn main() -> anyhow::Result<()> {
    ...
    tauri::Builder::new()
        ...
        .manage(SpotifyPollerContainer::default())
        .system_tray(
        ...
        .setup(|app| {
            ...
            async_runtime::spawn(async move {
                ...
                let (no_profile, enable_overlay) = {
                    let lock = config_container.0.lock().await;
                    (
                        lock.get_profiles().selected_profile.is_none(),
                        lock.get_preferences().enable_overlay,
                    )
                };
                ...
                handle.state::<SpotifyPollerContainer>().0.lock().await.start(handle.clone());
            });
