...
use self::{preferences::Preferences, profiles::Profiles, spotify_auth::SpotifyAuth};
...
impl ConfigManager {
    ...
    pub fn get_spotify_auth(&self) -> &SpotifyAuth {
        &self.spotify_auth
    }
    ...
    pub fn set_spotify_auth(&mut self, auth: SpotifyAuth) -> Result<()> {
        self.spotify_auth = auth;
        self.spotify_auth.write()
    }
}
