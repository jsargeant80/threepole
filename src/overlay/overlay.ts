...
import type { TauriEvent, Preferences, CurrentActivity, PlayerDataStatus, SpotifyTrack } from "../core/types";
...
const spotifyTrackElem = document.querySelector<HTMLElement>("#spotify-track")!;
const spotifyTitleElem = document.querySelector<HTMLElement>("#spotify-title")!;
const spotifyArtistElem = document.querySelector<HTMLElement>("#spotify-artist")!;
...
async function init() {
    ...
    appWindow.listen("spotify_track", (e: TauriEvent<SpotifyTrack | null>) => updateSpotifyTrack(e.payload));
}

function updateSpotifyTrack(track: SpotifyTrack | null) {
    if (track) {
        spotifyTitleElem.textContent = track.title;
        spotifyArtistElem.textContent = track.artist;
        spotifyTrackElem.classList.remove("hidden");
    } else {
        spotifyTrackElem.classList.add("hidden");
    }
}
